...
instance HasIniConfig 'QCustomProjection where
  ...
  getConfig content@(ConfigContent cfg) (Args'QCustomProjection mr) capabilities
      = do binocularsConfig'QCustom'Common <- getConfig content (Args'Common mr) capabilities
           ...
           let binocularsConfig'QCustom'SampleAxis = eitherF (const errorMissingSampleAxis) (parse' cfg "projection" "sampleaxis")
                                                     (\case
...
mk'Geometry'Path inputtype mWavelength cfg =
    ...
    in case inputtype of
      ...
      SixsFlyMedH -> dataSourcePath'Geometry'Sixs'MedH
      SixsFlyMedHGisaxs -> dataSourcePath'Geometry'Sixs'MedHGisaxs
      SixsFlyMedV -> dataSourcePath'Geometry'Sixs'MedV
      SixsFlyMedVGisaxs -> dataSourcePath'Geometry'Sixs'MedVGisaxs
      SixsFlyUhv -> dataSourcePath'Geometry'Sixs'Uhv
      SixsFlyUhvGisaxs -> dataSourcePath'Geometry'Sixs'UhvGisaxs
      SixsSbsMedH -> dataSourcePath'Geometry'Sixs'MedH
      SixsSbsMedHGisaxs -> dataSourcePath'Geometry'Sixs'MedHGisaxs
      SixsSbsMedV -> dataSourcePath'Geometry'Sixs'MedV
      SixsSbsMedVGisaxs -> dataSourcePath'Geometry'Sixs'MedVGisaxs
      SixsSbsUhv -> dataSourcePath'Geometry'Sixs'Uhv
...
mk'Mask'Path :: Config Common -> DSWrap_ DSMask DSPath
mk'Mask'Path c
...
guess'DataSource'DataFrameQCustom common msub cfg =
    do
      ...
      let dataSourcePath'DataFrameQCustom'Sixs'Fly :: DSWrap_ DSAttenuation DSPath -> DSWrap_ DSGeometry DSPath -> DSWrap_ DSImage DSPath -> DSWrap_ DSMask DSPath -> DSWrap_ DSDataFrameQCustom DSPath
          dataSourcePath'DataFrameQCustom'Sixs'Fly att g i m
              = [ DataSource'DataFrameQCustom
                  att
                  g
                  i
      ...
      let dataSourcePath'DataFrameQCustom'Sixs'Sbs :: DSWrap_ DSAttenuation DSPath -> DSWrap_ DSGeometry DSPath -> DSWrap_ DSImage DSPath -> DSWrap_ DSMask DSPath -> DSWrap_ DSDataFrameQCustom DSPath
          dataSourcePath'DataFrameQCustom'Sixs'Sbs att g i m
              = [ DataSource'DataFrameQCustom
                  att
                  g
                  i
      ...
      case inputtype of
         ...
         SixsFlyMedH -> dataSourcePath'DataFrameQCustom'Sixs'Fly attenuation'Path geometry'Path image'Path mask'Path
         SixsFlyMedHGisaxs -> dataSourcePath'DataFrameQCustom'Sixs'Fly attenuation'Path geometry'Path image'Path mask'Path
         SixsFlyMedV -> dataSourcePath'DataFrameQCustom'Sixs'Fly attenuation'Path geometry'Path image'Path mask'Path
         SixsFlyMedVGisaxs -> dataSourcePath'DataFrameQCustom'Sixs'Fly attenuation'Path geometry'Path image'Path mask'Path
         SixsFlyUhv -> dataSourcePath'DataFrameQCustom'Sixs'Fly attenuation'Path geometry'Path image'Path mask'Path
         SixsFlyUhvGisaxs -> dataSourcePath'DataFrameQCustom'Sixs'Fly attenuation'Path geometry'Path image'Path mask'Path
         SixsSbsMedH -> dataSourcePath'DataFrameQCustom'Sixs'Sbs attenuation'Path geometry'Path image'Path mask'Path
         SixsSbsMedHGisaxs -> dataSourcePath'DataFrameQCustom'Sixs'Sbs attenuation'Path geometry'Path image'Path mask'Path
         SixsSbsMedV -> dataSourcePath'DataFrameQCustom'Sixs'Sbs attenuation'Path geometry'Path image'Path mask'Path
         SixsSbsMedVGisaxs -> dataSourcePath'DataFrameQCustom'Sixs'Sbs attenuation'Path geometry'Path image'Path mask'Path
         SixsSbsUhv -> dataSourcePath'DataFrameQCustom'Sixs'Sbs attenuation'Path geometry'Path image'Path mask'Path
         SixsSbsUhvGisaxs -> dataSourcePath'DataFrameQCustom'Sixs'Sbs attenuation'Path geometry'Path image'Path mask'Path
...
spaceQCustom det pixels rs surf mlimits subprojection uqx uqy uqz mSampleAxis polarisation mdmask space@(Space fSpace) (DataFrameQCustom att g img mmask index timescan0 scannumber) =
  withGeometry g $ \c'geometry ->
  withForeignPtr (toForeignPtr pixels) $ \c'pix ->
  withForeignPtr fSpace $ \c'pSpace ->
  withMaybeDynamicMask mdmask $ \ c'dmask ->
  withMaybeMask mmask $ \ c'mask ->
  withMaybeLimits mlimits rs $ \nlimits c'limits ->
  withMaybeSampleAxis mSampleAxis $ \c'sampleAxis ->
  withNPixels det $ \c'nPixels ->
  withPixelsDims pixels $ \ndim c'dims ->
  withResolutions rs $ \nr c'r -> do
    let c'polarisation = fromBool . unPolarisationCorrection $ polarisation
    let c'attenuation = CDouble . unAttenuation $ att
    let c'ndim = toEnum ndim
    let c'nr = toEnum nr
    let c'surf = toEnum . fromEnum $ surf
    let c'nlimits = toEnum nlimits
    let c'timestamp = CDouble . unTimestamp $ index
    let c'timescan0 = CDouble . unTimescan0 $ timescan0
    let c'scannumber = toEnum . fromEnum . unScannumber $ scannumber
    let c'subprojection = toEnum . fromEnum $ subprojection
    let c'uqx = CDouble (uqx /~ radian)
    let c'uqy = CDouble (uqy /~ radian)
    let c'uqz = CDouble (uqz /~ radian)
    case img of
      (ImageDouble arr) -> unsafeWith arr $ \i -> do
                                 {-# SCC "hkl_binoculars_space_qcustom_double" #-} c'hkl_binoculars_space_qcustom_double c'pSpace c'geometry i c'nPixels c'attenuation c'pix c'ndim c'dims c'r c'nr c'mask c'surf c'limits c'nlimits c'timestamp c'timescan0 c'scannumber c'subprojection c'uqx c'uqy c'uqz c'sampleAxis c'polarisation c'dmask
      (ImageInt32 arr) -> unsafeWith arr $ \i -> do
                                 {-# SCC "hkl_binoculars_space_qcustom_int32_t" #-} c'hkl_binoculars_space_qcustom_int32_t c'pSpace c'geometry i c'nPixels c'attenuation c'pix c'ndim c'dims c'r c'nr c'mask c'surf c'limits c'nlimits c'timestamp c'timescan0 c'scannumber c'subprojection c'uqx c'uqy c'uqz c'sampleAxis c'polarisation c'dmask
      (ImageWord16 arr) -> unsafeWith arr $ \i -> do
                                 {-# SCC "hkl_binoculars_space_qcustom_uint16_t" #-} c'hkl_binoculars_space_qcustom_uint16_t c'pSpace c'geometry i c'nPixels c'attenuation c'pix c'ndim c'dims c'r c'nr c'mask c'surf c'limits c'nlimits c'timestamp c'timescan0 c'scannumber c'subprojection c'uqx c'uqy c'uqz c'sampleAxis c'polarisation c'dmask
      (ImageWord32 arr) -> unsafeWith arr $ \i -> do
                                 {-# SCC "hkl_binoculars_space_qcustom_uint32_t" #-} c'hkl_binoculars_space_qcustom_uint32_t c'pSpace c'geometry i c'nPixels c'attenuation c'pix c'ndim c'dims c'r c'nr c'mask c'surf c'limits c'nlimits c'timestamp c'timescan0 c'scannumber c'subprojection c'uqx c'uqy c'uqz c'sampleAxis c'polarisation c'dmask
